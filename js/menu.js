class MenuManager {
    ...
    async fetchMenuData() {
        try {
            const response = await fetch(
                `${CONFIG.SHEETS_API_ENDPOINT}/${CONFIG.SHEET_ID}/values/${CONFIG.SHEET_NAME}?key=${CONFIG.API_KEY}`
            );
            const data = await response.json();
            this.menuData = this.processSheetData(data.values);
        } catch (error) {
    ...
    processSheetData(values) {
        ...
        const items = values.slice(1).map(row => ({
            name: row[CONFIG.COLUMNS.DISH_NAME],
            ...
            qrCode: row[CONFIG.COLUMNS.QR_CODE]
        }));
    ...
    createItemCard(item) {
        ...
        card.innerHTML = `
            <div class="item-image">
                <img src="${item.imageUrl}" alt="${item.name}" onerror="this.src='assets/placeholder.svg'">
            </div>
